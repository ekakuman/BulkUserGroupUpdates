...
def update_group_with_user_ids(access_token, group_id, user_ids):
    ...
    for count, (email, user_id) in enumerate(user_ids.items(), 1):
        ...
        if response.status_code in (200, 201, 204):
            if response.status_code == 200 and items:
                ...
                print(f"{count}, user_id: {user_id}, email: {email} - User has been created.")
                successful_emails.append(email)
            elif response.status_code == 202:
                print(f"{count}, user_id: {user_id}, email: {email} - Update has been accepted for processing.")
                successful_emails.append(email)
            elif response.status_code == 204:
                print(f"{count}, user_id: {user_id}, email: {email} - Successful request without body content.")
                successful_emails.append(email)
            else:
                print(f"{count}, user_id: {user_id}, email: {email} - Failed to update. Empty response.\n")
            ...
            if response.status_code in (200, 201, 204) and items:
                ...
                print(f"{count}, user_id: {user_id}, email: {email} - Failed to update even after retry. Status code: {response.status_code}\n")
                failed_emails.append(email)
        else:
    ...
    print(f"\nTotal number of users successfully added to {group_id}: {successful_count}\n")
